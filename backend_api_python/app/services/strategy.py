...
_SECRET_KEYS = {
    ...
    "password", "mt5_password", "private_key", "privatekey", "access_token", "accesstoken",
    ...
    def test_exchange_connection(self, exchange_config: Dict[str, Any], user_id: int = 1) -> Dict[str, Any]:
        try:
            ...
            if exchange_id in {"mt5", "cptmarkets", "cpt_markets"}:
                market_category = str(resolved.get("market_category") or "").strip()
                if market_category and market_category != "Forex":
                    return {
                        "success": False,
                        "message": "MT5/CPT Markets requires market_category=Forex",
                        "data": None,
                    }
            client = create_client(resolved, market_type=str(resolved.get("market_type") or "swap"))
